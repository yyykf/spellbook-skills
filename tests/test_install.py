...
覆盖：target 选择、合并不破坏既有、幂等、精确卸载、Copilot 用户内容保留、稳定位置 copy，
...
def main():
    ...
    def run(action, *args):
        subprocess.run([sys.executable, INSTALL_PY, action, *args], env=env,
                       capture_output=True, text=True, check=True)

    def run_fails(action, *args):
        return subprocess.run([sys.executable, INSTALL_PY, action, *args], env=env,
                              capture_output=True, text=True).returncode != 0

    def write_fake_codex(output, *, exit_code=0):
        path = os.path.join(fake_bin, "codex")
    ...
    print("=== install 默认只安装 Copilot（Codex 0.137.0+ 走插件 hook）===")
    run("install")
    ss = codex_ss()
    ...
    check(len(ours) == 0, f"默认幂等：Codex fallback hook 仍 0 条（实际 {len(ours)}）")
    check(len(ss) == 1, f"默认幂等：SessionStart 仍 1 条（实际 {len(ss)}）")
    ...
    print("=== target=all 安装 Codex fallback + Copilot ===")
    reset(codex=json.dumps({"hooks": {
        "SessionStart": [{"hooks": [{"type": "command", "timeout": 5,
            "command": "/Users/code4j/.codeisland/codeisland-bridge --source codex"}]}],
        "Stop": [{"hooks": [{"type": "command", "command": "echo stop"}]}],
    }}, indent=2), copilot="# 我的全局规则\n始终用中文。\n")
    ...
    reset(codex='{"hooks": {"SessionStart": []}}', copilot="# 用户 Copilot 规则\n")
    run("install", "--target", "copilot")
    check(codex_hook_count() == 0, "target=copilot：不写 Codex hook")
    check("BEGIN spellbook-skills" in open(copilot_file).read(), "target=copilot：写入 Copilot instructions")
    run("uninstall", "--target", "copilot")
    check(codex_hook_count() == 0, "target=copilot：卸载不触碰 Codex hook")
    check("spellbook-skills" not in open(copilot_file).read(), "target=copilot：卸载移除 Copilot marker")

    print("=== target=auto：旧 Codex 自动安装 fallback ===")
    reset(codex='{"hooks": {"SessionStart": []}}', copilot="# 用户 Copilot 规则\n")
    ...
    print("=== target=auto：无法判断版本时保守失败、不写配置 ===")
    reset(codex='{"hooks": {"SessionStart": []}}', copilot="# 用户 Copilot 规则\n")
    write_fake_codex("codex development build")
    check(run_fails("install", "--target", "auto"), "target=auto：版本不可识别时非 0 退出")
    check(codex_hook_count() == 0, "target=auto：不可识别版本不写 Codex fallback")
    check("spellbook-skills" not in open(copilot_file).read(), "target=auto：不可识别版本不写 Copilot")
    check(not os.path.isdir(env["SPELLBOOK_HOME"]), "target=auto：不可识别版本不 copy payload")
    check(run_fails("status", "--target", "auto"), "target=auto：status 非 0 退出")
    check(run_fails("uninstall", "--target", "auto"), "target=auto：uninstall 非 0 退出")
    ...
    reset()
    run("install")
    check(not os.path.exists(codex_file), "全新环境默认 install 不创建 Codex hooks.json")
    check("BEGIN spellbook-skills" in open(copilot_file).read(), "全新环境默认 install 写入 Copilot instructions")
    run("uninstall")
    check(not os.path.exists(copilot_file), "全新环境默认 uninstall 后 Copilot 文件无残留")
    ...
    check(run_fails("install", "--target", "codex-fallback"), "损坏 JSON：codex-fallback install 非 0 退出")
